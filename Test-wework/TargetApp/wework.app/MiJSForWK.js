...
var vsprintf = function(fmt, argv) {
  ...
  return sprintf.apply(null, argv);
    ...
    function _sendMessage(message) {
        ...
        window.webkit.messageHandlers.shenpiDispatchMessage.postMessage(msInQueue);
    };
    //取出队列中的消息，并清空接收队列
    ...
    function _log(fmt) {
      ...
      try {
        ...
        msg = fmt;
                                                                                                                                                                                                            ...
                                                                                                                                                                                                            _call('utility.log',{'msg':msg},function(){});
    }

    function _call(func,params,callback) {
        if (func !== 'utility.log') {                                                                                                                                                                _log('call func:%s', func);
        };
    ...
    function _enable_old_UrlStyleImagePreviews() {
      ...
      _MiJS('a[href^="mi://viewimage/"]').on('click',function(e){
        var cur = '';
        var link;
        if (typeof e.target.href === 'string' && e.target.href.search(old_prefix) === 0) {
          link = e.target;
        }else{
          link = _MiJS(e.target).parents('a[href^="mi://viewimage/"]')[0];
        }
        cur = link.href.substr(old_prefix.length);
        var allLinks = _MiJS('a[href^="mi://viewimage/"]');
        var allUrls = [];
    ...
    function _enable_old_ReaderShareUrls() {
      ...
      _MiJS('a[href^="mi://readershare/"]').on('click',function(e){
        e.preventDefault();     

        _emit('menu:share:weibo',_session_data.shareWeiboData || {});
      });
      _MiJS('a[href^="mi://readertimeline/"]').on('click',function(e){
        e.preventDefault();
    ...
    function _setDefaultEventHandlers() {
      ...
      _on('menu:setfont',function(argv){
        ...
        if(argv.isFirstAutoSet && argv.fontSize === '2') {
          return;
        }
