...
var vsprintf = function(fmt, argv) {
  ...
  return sprintf.apply(null, argv);
    ...
    var _sendMessageQueue = [], // page to native的消息队列
        ...
        _QUEUE_HAS_MESSAGE_URL = 'rtx://dispatch_message/';
    ...
    function _enable_old_UrlStyleImagePreviews() {
      var old_prefix = "rtx://viewimage/";
      _MiJS('a[href^="rtx://viewimage/"]').on('click',function(e){
        var cur = '';
        var link;
        if (typeof e.target.href === 'string' && e.target.href.search(old_prefix) === 0) {
          link = e.target;
        }else{
          link = _MiJS(e.target).parents('a[href^="rtx://viewimage/"]')[0];
        }
        cur = link.href.substr(old_prefix.length);
        var allLinks = _MiJS('a[href^="rtx://viewimage/"]');
        var allUrls = [];
    ...
    function _enable_old_ReaderShareUrls() {
      var old_prefix = "rtx://readershare/";
      _MiJS('a[href^="rtx://readershare/"]').on('click',function(e){
        e.preventDefault();     

        _emit('menu:share:weibo',_session_data.shareWeiboData || {});
      });
      _MiJS('a[href^="rtx://readertimeline/"]').on('click',function(e){
        e.preventDefault();
