...
//     _MiJS.js may be freely distributed under the MIT license.
(function() {
    // if (window.MiJSBridge) {
    //     return;
    // };

   /* _MiJS v1.0rc1 - polyfill _MiJS event detect fx ajax form touch - _MiJSjs.com/license */
;(function(undefined){
...
var _MiJS = (function() {
  var undefined, key, $, classList, emptyArray = [], slice = emptyArray.slice,
    ...
    // Used by `$._MiJS.init` to wrap elements, text/comment nodes, document,
  ...
  _MiJS.matches = function(element, selector) {
    ...
    match = ~_MiJS.qsa(parent, selector).indexOf(element)
  ...
  // `$._MiJS.fragment` takes a html string and an optional tag name
  // to generate DOM nodes nodes from the given html string.
  // The generated DOM nodes are returned as an array.
  // This function can be overriden in plugins for example to make
  // it compatible with browsers that don't support the DOM fully.
  _MiJS.fragment = function(html, name) {
  ...
  // `$._MiJS.Z` swaps out the prototype of the given `dom` array
  // of nodes with `$.fn` and thus supplying all the _MiJS functions
  // to the array. Note that `__proto__` is not supported on Internet
  // Explorer. This method can be overriden in plugins.
  _MiJS.Z = function(dom, selector) {
    dom = dom || []
  ...
  // `$._MiJS.isZ` should return `true` if the given object is a _MiJS
  // collection. This method can be overriden in plugins.
  _MiJS.isZ = function(object) {
    return object instanceof _MiJS.Z
  }

  // `$._MiJS.init` is _MiJS's counterpart to jQuery's `$.fn.init` and
  // takes a CSS selector and an optional context (and handles various
  // special cases).
  // This method can be overriden in plugins.
  _MiJS.init = function(selector, context) {
    // If nothing given, return an empty _MiJS collection
    if (!selector) return _MiJS.Z()
    // If a function is given, call it when the DOM is ready
    else if (isFunction(selector)) return $(document).ready(selector)
    // If a _MiJS collection is given, juts return it
    else if (_MiJS.isZ(selector)) return selector
    else {
      var dom
      ...
      // If it's a html fragment, create nodes from it
        ...
        dom = _MiJS.fragment(selector.trim(), RegExp.$1), selector = null
      // If there's a context, create a collection on that context first, and select
      // nodes from there
      else if (context !== undefined) return $(context).find(selector)
      // And last but no least, if it's a CSS selector, use it to select nodes.
      else dom = _MiJS.qsa(document, selector)
      // create a new _MiJS collection from the nodes found
      return _MiJS.Z(dom, selector)
    }
  }

  // `$` will be the base `_MiJS` object. When calling this
  // function just call `$._MiJS.init, whichs makes the implementation
  // details of selecting nodes and creating _MiJS collections
  // patchable in plugins.
  $ = function(selector, context){
    return _MiJS.init(selector, context)
  }
  ...
  // `$._MiJS.qsa` is _MiJS's CSS selector implementation which
  // uses `document.querySelectorAll` and optimizes for some special cases, like `#id`.
  // This method can be overriden in plugins.
  _MiJS.qsa = function(element, selector){
    var found
  ...
  $.fn = {
    ...
    filter: function(selector){
      return $([].filter.call(this, function(element){
        return _MiJS.matches(element, selector)
      }))
    },
    ...
    is: function(selector){
      return this.length > 0 && _MiJS.matches(this[0], selector)
    },
    ...
    find: function(selector){
      var result
      if (this.length == 1) result = _MiJS.qsa(this[0], selector)
      else result = this.map(function(){ return _MiJS.qsa(this, selector) })
      return $(result)
    },
    closest: function(selector, context){
      var node = this[0]
      while (node && !_MiJS.matches(node, selector))
  ...
  adjacencyOperators.forEach(function(key, operator) {
    $.fn[key] = function(){
      // arguments can be nodes, arrays of nodes, _MiJS objects and HTML strings
      var nodes = $.map(arguments, function(n){ return isObject(n) ? n : _MiJS.fragment(n) })
  ...
  _MiJS.Z.prototype = $.fn

  // Export internal API functions in the `$._MiJS` namespace
  _MiJS.camelize = camelize
  _MiJS.uniq = uniq
  $._MiJS = _MiJS
...
// If `$` is not yet defined, point it to `_MiJS`
window._MiJS = _MiJS
// '$' in window || (window.$ = _MiJS)
;(function($){
  var $$ = $._MiJS.qsa, handlers = {}, _zid = 1, specialEvents={}
...
k=j.BufferedBlockAlgorithm=f.extend({reset:function(){this._data=new n.init;this._nDataBytes=0},_append:function(a){"string"==typeof a&&(a=r.parse(a));this._data.concat(a);this._nDataBytes+=a.sigBytes},_process:function(a){var c=this._data,b=c.words,d=c.sigBytes,f=this.blockSize,h=d/(4*f),h=a?e.ceil(h):e.max((h|0)-this._minBufferSize,0);a=h*f;d=e.min(4*a,d);if(a){for(var g=0;g<a;g+=f)this._doProcessBlock(b,g);g=b.splice(0,a);c.sigBytes-=d}return new n.init(g,d)},clone:function(){var a=f.clone.call(this);
    ...
    var _sendMessageQueue = [], // page to native的消息队列
        ...
        _QUEUE_HAS_MESSAGE_URL = ‘mi://dispatch_message/';
    ...
    function _handleMessageFromMi(originmessage) {
    ...
    function _disableImageSelection() {
        ...
        for (var i = 0; i < imgs.length; i++) {
          _MiJS(imgs[i])._MiJS_old_touch_callout = _MiJS(imgs[i]).css('-webkit-touch-callout');
          _MiJS(imgs[i])._MiJS_old_user_select = _MiJS(imgs[i]).css('-webkit-user-select');
        };
        _MiJS('img').css({
    ...
    function _restoreImageSelection() {
        var imgs = _MiJS('img');
        for (var i = 0; i < imgs.length; i++) {
            if (typeof _MiJS(imgs[i])._MiJS_old_touch_callout != 'undefined') {
                _MiJS(imgs[i]).css({
                    '-webkit-touch-callout': _MiJS(imgs[i])._MiJS_old_touch_callout,
                    '-webkit-user-select': _MiJS(imgs[i])._MiJS_old_user_select
                });
            }
        };
    }
    ...
    function _disableAlertView() {
        window.__MiJS_sys_alert = window.alert;
        window.alert = null;
        window.__MiJS_sys_prompt = window.prompt;
        window.prompt = null;
    }

    function _restoreAlertView() {
        window.alert = window.__MiJS_sys_alert;
        window.prompt = window.__MiJS_sys_prompt;
        delete window.__MiJS_sys_alert;
        delete window.__MiJS_sys_prompt;
    }
    ...
    function _setDefaultEventHandlers() {
      ...
      var getSharePreviewImage = function(cb){
        ...
        for (var i = 0; i < _allImgs.length; i++) {
          ...
          // 过滤掉不可以见的图片
          if (_MiJS(_img).css('display')=='none' || _MiJS(_img).css('visibility')=='hidden') {
...
//        emit:_emit,
    ...
    // Copy all but undefined properties from one or more objects to the `window.MiJSBridge` object.
    if (window.MiJSBridge) {
      _MiJS.extend(window.MiJSBridge,__MiJSBridge);
    }else{
    ...
    _MiJS.JSON = JSON;
    _MiJS.disableImageSelection = _disableImageSelection;
    _MiJS.restoreImageSelection = _restoreImageSelection;
    _MiJS.disableAlertView = _disableAlertView;
    _MiJS.restoreAlertView = _restoreAlertView;

    // _MiJS("document").ready(function(){
    //     _call("domReady");
    // });
